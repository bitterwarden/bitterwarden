...
async function buildExtension() {
  ...
  await cp(join(extensionDir, 'manifest.json'), join(buildDir, 'manifest.json'));
  await cp(join(extensionDir, 'popup.html'), join(buildDir, 'popup.html'));
  await cp(join(extensionDir, 'options.html'), join(buildDir, 'options.html'), { force: true }).catch(() => {});

...
describe("SecureStorageService", () => {
	...
	it("should handle vault operations", async () => {
		...
		mockAsyncStorageGetItem.mockImplementation(() => Promise.resolve(JSON.stringify(encryptedVault)));
		const result = await service.getVault();
		expect(result).toEqual(encryptedVault);

		// Test clear
		await service.clearVault();
		expect(mockAsyncStorageRemoveItem).toHaveBeenCalledWith("@bitterwarden/vault");
	});
	...
	it("should handle secure store operations", async () => {
		...
		mockSecureStoreGetItemAsync.mockImplementation(() => Promise.resolve("hash123"));
		const hash = await service.getMasterPasswordHash();
		expect(hash).toBe("hash123");
		expect(mockSecureStoreGetItemAsync).toHaveBeenCalledWith("@bitterwarden/master-hash");
	});
